...
$(function(){
	...
	$('#pro_click').click(function(){
		$('#pro_click_show').show();
		$('#mil1_click_show').hide();
		$('#mil2_click_show').hide();
		$('#mil3_click_show').hide();
	})
	
	$('#mil1_clikc').click(function(){
		$('#pro_click_show').hide();
		$('#mil1_click_show').show();
		$('#mil2_click_show').hide();
		$('#mil3_click_show').hide();
	})
	
	$('#mil2_clikc').click(function(){
		$('#pro_click_show').hide();
		$('#mil1_click_show').hide();
		$('#mil2_click_show').show();
		$('#mil3_click_show').hide();
	})
	
	$('#mil3_clikc').click(function(){
		$('#pro_click_show').hide();
		$('#mil1_click_show').hide();
		$('#mil2_click_show').hide();
		$('#mil3_click_show').show();
	})
	

	
	$(".most_popular_high").click(function() {
		$(".most_popular_high").each(function(){
			$(this).css('color','#434b659e');
		})
		$(this).css('color','#434b65');
	  });


	//$('.treeview-colorful').mdbTreeview();
	//display_projects_preivew(523)
	get_records(cpage);
	
	$('#internships_search').submit(function(){
		get_records(1);
	})
	$('.internship_roles').change(function(){
		var value = $(this).val();
		if(value == ""){
			$(this).css('background-color','#ffff');
		}
		else{
			$(this).css('background-color','#deffe3');
		}		
		get_records(1);
	...
	$('#clear_filter').click(function(){
		$('.internship_roles').each(function(){
			$(this).val('');
			$(this).css('background-color','#ffff');
		})
		get_records(1);
	...
	$("#search_filters_projects").keypress(function(e) {
		if(e.which == 13) {
			get_records(1);
		}
	});
...
function get_records(page)
{
    ...
    var skills = $.trim($('#skills_search').val()); 
    var search_filters_projects = $.trim($('#search_filters_projects').val()); 
    var price = '';
    var most_relavent = $.trim($('#most_relavent').val()); 
    
	$.ajax({
		url : base_url+"Student_API/guided_projects",
		method : "POST",
		data:{'Role':'records','page':page,'category':category,'subcategory':subcategory,'complexity':complexity,'skills':skills,'search_filters_projects':search_filters_projects,'price':price,'most_relavent':most_relavent},
		success:function(data){
			...
			if(data.records!='')
			{
				...
				$.each(data.records, function(i){
					...
					//var pro_tile = pro_tile.substring(0, 70);
					var short_desc = row.short_desc;
					var short_desc = short_desc.substring(0, 150);
					var skills = row.skills;
					var res = skills.split(",");
					var random_starts_loop = rondom_starts = (Math.random()*(49-41)+41)/10;
					random_starts_loop = Math.round(random_starts_loop * 10) / 10
					var minNumber = 80;
					var maxNumber = 100
					var rondom_starts = Math.floor(Math.random()*(maxNumber-minNumber+1)+minNumber);

 
					html += '<div class="col-md-6">';
					html += '<a href="'+base_url+'guided-project/'+row.seo_url+'" class="guide-list mt-4">';
					html += '<div class="row">';
					html += '<div class="col-md-4 col-4"><img class="project_image_urls" src="'+image_url+'" alt="big-badge-ibm"></div>';
					html += '<div class="col-md-8 p-0 col-8 project_title_content">';
					html += '<h6 class="pro_title">'+pro_tile.toLowerCase()+'</h6>';
					html += '<p class="short_desc">'+short_desc+'</p>';

					html += '</div>';
					html += '<div class="col-md-12">';
					html += '<p class="skils_btns">';
					for(z=0; z<res.length; z++){
						html += '<button class="btn mt-3">'+res[z]+'</button> &nbsp;';
					}
					html += '</p>';
					html += '</div>';
					html += '<div class="col-md-6 col-6" >';
					html += '<ul class="star-ul mt-4">';
					html += '<li class="mr-1"><h6 class="star-p">'+random_starts_loop+'</h6></li>';

					html += '<li><img class="star-li px-1" src="'+base_url+'assets/guided_projects/images/star-fill-mini.png" alt="star-ratting"></li>';
					html += '<li><img class="star-li px-1" src="'+base_url+'assets/guided_projects/images/star-fill-mini.png" alt="star-ratting"></li>';
					html += '<li><img class="star-li px-1" src="'+base_url+'assets/guided_projects/images/star-fill-mini.png" alt="star-ratting"></li>';
					html += '<li><img class="star-li px-1" src="'+base_url+'assets/guided_projects/images/star-fill-mini.png" alt="star-ratting"></li>';
					html += '<li><img class="star-li px-1" src="'+base_url+'assets/guided_projects/images/half-star.png" alt="star-ratting"></li>';

					html += '<span class="star-sp px-1 ">('+rondom_starts+')</span><br>';
					html += '</ul>';
					html += '</div>';
					html += '<br><br>';
					html += '<div class="col-md-6 col-6  pl-0 mt-3">';
					html += '<p class="mt-2 float-right">';
					html += '<span class="mr-2"><img class="mr-1 guide-neg" src="'+base_url+'assets/guided_projects/new/images/time.svg" alt="arrow-right">'+row.duration+' Hrs</span>';

					if(row.complexity == 'Basic'){
						html += '<span class="mr-2"><img class="mr-1 guide-neg" src="'+base_url+'assets/guided_projects/new/images/beginner.svg" alt="arrow-right">'+row.complexity+'</span>';
					}
					else if(row.complexity == 'Intermediate'){
						html += '<span class="mr-2"><img class="mr-1 guide-neg" src="'+base_url+'assets/guided_projects/new/images/beginner2.svg" alt="arrow-right">'+row.complexity+'</span>';
					}
					else if(row.complexity == 'Advanced'){
						html += '<span class="mr-2"><img class="mr-1 guide-neg" src="'+base_url+'assets/guided_projects/new/images/beginner3.svg" alt="arrow-right">'+row.complexity+'</span>';
					}
					else{
						html += '<span class="mr-2"><img class="mr-1 guide-neg" src="'+base_url+'assets/guided_projects/new/images/beginner.svg" alt="arrow-right">'+row.complexity+'</span>';
					}
				...
				html+='<div class="text-center no_res_search col-md-12 col-12">';
				html+='<img src="'+base_url+'assets/guided_projects/images/no_search.png" class="no_result_img">';
				html+="<h3 class='no_result_img_h3' >Sorry, we couldn't find any projects on this Search Filter. </h3 >"  ;
				html += '</div>';
			}
...
function display_projects_preivew(last_id){
	...
	$.ajax({
		url : base_url+"Company_API/admin_get_project_view",
		type : "POST",
...
function get_project_data(projectId){
	$.ajax({
		url : base_url+"Company_API/admin_get_project_data_by_id",
		type : "POST",
...
function get_mile_stone_data(milestone_data, projectId){
	$.ajax({
		url : base_url+"Company_API/admin_get_mile_stone_data",
		type : "POST",
...
function get_activity_details(actvityId){
	$.ajax({
			url : base_url+"Company_API/admin_get_activity_data",
			type : "POST",
...
function get_refrences(referenceId){
	...
	$.ajax({
			url : base_url+"Company_API/admin_get_refrence_data",
			type : "POST",
